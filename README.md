...
A logged in user has the three properties session["user_id] and session["user_section"] set to the user's id and section (from the database). In addition, session["weeks_visible"], which determines if the sidebar is displayed, is set to True - it is set to False by any route that requires the sidebar to be toggled off (such as by change_pw, logout, and usermgmt).
...
##### static/docs
Contains the assignment files that are displayed to the user in assignments.html by "/assignments"
